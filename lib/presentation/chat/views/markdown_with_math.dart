...
class MarkdownWithMath extends StatelessWidget {
  ...
  const MarkdownWithMath({Key? key, required this.markdownData}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return MarkdownBody(
      ...
      builders: {'inline_math': InlineMathBuilder(), 'block_math': NewlineMathBuilder()},
...
class MathInlineSyntax extends md.InlineSyntax {
  ...
  bool onMatch(md.InlineParser parser, Match match) {
    ...
    final element = md.Element.text("inline_math", mathExpression);
...
class MathNewlineSyntax extends md.InlineSyntax {
  ...
  bool onMatch(md.InlineParser parser, Match match) {
    ...
    final element = md.Element.text("block_math", mathExpression);
    parser.addNode(element); // Add the custom math node
    return true;
  }
}
